  ...
  'application/gzip': (imagePath, options) => {
    return Bluebird.using(fs.openAsync(imagePath, 'r').disposer((fileDescriptor) => {
      ...
      return gzip.getUncompressedSize({
        size: options.size,
        read: (count, offset) => {
          return utils.readBufferFromImageFileDescriptor(fileDescriptor, count, offset);
        }
      });
    ...
    return Bluebird.using(fs.openAsync(imagePath, 'r').disposer((fileDescriptor) => {
      ...
      return lzma.parseFileIndexAsync({
        ...
        read: (count, offset, callback) => {
          utils.readBufferFromImageFileDescriptor(fileDescriptor, count, offset).asCallback(callback);
        }
      });
    }).then((metadata) => {
      return {
