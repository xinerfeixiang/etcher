 ...
 * @param {Function} options.read - read function (count, offset)
 ...
 *     fs.readSync(fd, buffer, 0, count, offset);
 *     return buffer;
 *   }
...
exports.getUncompressedSize = (options) => {
  ...
  const ISIZE_OFFSET = options.size - ISIZE_LENGTH;
  return options.read(ISIZE_LENGTH, ISIZE_OFFSET).then((buffer) => {
